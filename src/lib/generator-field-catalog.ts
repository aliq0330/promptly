...
import { placeholderArt } from "./placeholder-image";
 ...
 * each gets a deterministic, fully offline thumbnail generated by this
 * app's own existing `placeholderArt()` (already used for prompt-media
 * placeholders, CLAUDE.md's mock-media precedent — no new image system, no
 * external image service, no network fetch). This is a real "default
 * image" in the sense the spec asks for (a creator adding this field never
 * has to upload anything themselves), but it is honestly an abstract,
 * auto-generated visual — not a curated stock photo (this sandbox has no
 * way to source/generate real representative photography). A creator can
 * always replace it with a real uploaded photo, scoped to their own
 * generator only (`FieldEditorModal`'s "Görseli değiştir"/"Görseli
 * kaldır"), which never touches this shared catalog default.
 */
function imgOpts(fieldSeed: string, labels: string[]): CatalogOption[] {
  return labels.map((label) => ({ label, value: slugValue(label), image: placeholderArt(`${fieldSeed}:${label}`, 160, 160) }));
}
