 ...
 *
 * DEFAULT VISUAL OPTIONS (CLAUDE.md "Generator Hazır Alanları + Varsayılan
 * Görsel Seçenekleri"): `GeneratorFieldOption` grew two OPTIONAL, additive
 * fields — `image`/`color` — so a hazır alan's options can carry a real
 * visual default the moment they're inserted, with no upload required.
 * `color` is real hex data for genuinely literal color-name fields (Saç
 * Rengi, Göz Rengi, …), matching the spec's own example verbatim. `image`
 * is a deterministic, fully offline thumbnail from this app's existing
 * `placeholderArt()` (the same utility mock prompt media has always used —
 * no new image system, no external image service, no network fetch) for a
 * representative set of the spec's own named examples (Saç Tipi/Uzunluğu,
 * Göz Şekli, Yüz Şekli, Kıyafet, Poz, Kamera Açısı, Fotoğraf Türü) — NOT
 * every image-eligible field in this file (same "genuinely useful starting
 * point, not exhaustive" scope decision as the catalog's overall breadth
 * above). These are honest, auto-generated visual placeholders, not
 * curated stock photography; a creator can replace either one with a real
 * upload, scoped to their own generator's copy of the option only — see
 * `field-editor-modal.tsx`. Neither field is ever read by prompt
 * generation (`generator-output.ts` only ever reads `value`/`label`), and
 * both are fully optional — a generator saved before this feature existed
...
export interface CatalogOption {
  label: string;
  value: string;
  /** See `GeneratorFieldOption.image` (types/index.ts) — carried through verbatim when a catalog field is inserted into a generator's real schema (`generator-builder.tsx`'s `insertFieldDescriptors`). */
  image?: string;
  /** See `GeneratorFieldOption.color` (types/index.ts). */
  color?: string;
}
...
/**
 * Options for a literal color-name field (Saç Rengi, Göz Rengi, …) — each
 * gets a real hex swatch alongside its label/value, per the user's own
 * "PROMPTLY GENERATOR — HAZIR ALANLARI + VARSAYILAN GÖRSEL SEÇENEKLERİ"
 * request (§5/§6). The hex is a real, sensible default a creator can
 * override per-generator (`FieldEditorModal`'s option editor) — it is never
 * read by prompt generation, only ever `value`/`label` are (unchanged,
 * `generator-output.ts`'s `describeFieldValue`).
 */
function colorOpts(pairs: [label: string, hex: string][]): CatalogOption[] {
  return pairs.map(([label, hex]) => ({ label, value: slugValue(label), color: hex }));
}

/**
 * Options for a visually-distinct field (Saç Şekli, Poz, Kamera Açısı, …) —
 * each gets a deterministic, fully offline thumbnail from
 * `generator-option-art.ts`'s hand-drawn, per-concept line-art icon set
 * (a curly-hair icon for "Kıvırcık", a low-angle camera diagram for "Alttan
 * Açı", …) — no new image system, no external image service, no network
 * fetch. **Revision note:** this used to call the app's generic, abstract
 * `placeholderArt()` (colored gradient "bokeh" blobs, still used for mock
 * prompt media elsewhere) — an explicit user report said those blobs looked
 * unrelated to the field they represented, so this now uses real,
 * concept-specific icons instead (see `generator-option-art.ts`'s own doc
 * comment for the honest scope note: still hand-drawn line art, not an
 * actual photograph — this sandbox cannot generate/source real photography).
 * A creator can always replace any option's icon with a real uploaded
 * photo, scoped to their own generator only (`FieldEditorModal`'s "Görseli
 * değiştir"/"Görseli kaldır"), which never touches this shared catalog
 * default.
 */
function imgOpts(fieldSeed: string, labels: string[]): CatalogOption[] {
  return labels.map((label) => ({ label, value: slugValue(label), image: conceptIcon(fieldSeed, label, 160, 160) }));
}
...
export const CATALOG_FIELDS: CatalogField[] = [
  ...
  { id: "char_face_shape", label: "Yüz Şekli", type: "select", categoryId: "character", subgroupId: "identity", jsonPath: "face.shape", options: imgOpts("char_face_shape", ["Oval", "Yuvarlak", "Kare", "Kalp", "Elmas", "Uzun"]) },
  { id: "char_skin_tone", label: "Ten Rengi", type: "select", categoryId: "character", subgroupId: "skin", jsonPath: "skin.tone", options: colorOpts([["Çok Açık", "#F3DCC6"], ["Açık", "#E8C29E"], ["Buğday", "#C99770"], ["Esmer", "#A9714A"], ["Koyu", "#7A4B2E"], ["Çok Koyu", "#4A2E1C"]]) },
  ...
  { id: "char_eye_color", label: "Göz Rengi", type: "select", categoryId: "character", subgroupId: "eyes", jsonPath: "face.eyes.color", options: colorOpts([["Mavi", "#3B6EA5"], ["Yeşil", "#3F7A4E"], ["Kahverengi", "#5A3825"], ["Gri", "#8C9096"], ["Ela", "#7A6A45"], ["Amber", "#B5772E"], ["Mor", "#6B3FA0"], ["Kırmızı", "#A3262A"], ["Siyah", "#1C1310"]]) },
  { id: "char_eye_shape", label: "Göz Şekli", type: "select", categoryId: "character", subgroupId: "eyes", jsonPath: "face.eyes.shape", options: imgOpts("char_eye_shape", ["Badem", "Yuvarlak", "Çekik", "Büyük", "Küçük", "Derin Set"]) },
  ...
  { id: "char_eyebrow_color", label: "Kaş Rengi", type: "select", categoryId: "character", subgroupId: "eyebrows", jsonPath: "face.eyebrows.color", options: colorOpts([["Siyah", "#1C1310"], ["Kahverengi", "#5A3825"], ["Sarı", "#D6B56A"], ["Kızıl", "#A33A24"], ["Gri", "#9B9B93"], ["Beyaz", "#F5F3EE"]]) },
  ...
  { id: "char_lip_color", label: "Dudak Rengi", type: "select", categoryId: "character", subgroupId: "mouth", jsonPath: "face.mouth.lip_color", options: colorOpts([["Doğal", "#C88A72"], ["Pembe", "#D98CB3"], ["Kırmızı", "#B3231F"], ["Bordo", "#6E1F26"], ["Mor", "#6B3FA0"], ["Koyu", "#4A2A28"]]) },
  ...
  { id: "char_hair_length", label: "Saç Uzunluğu", type: "select", categoryId: "character", subgroupId: "hair", jsonPath: "hair.length", options: imgOpts("char_hair_length", ["Kel", "Çok Kısa", "Kısa", "Orta", "Uzun", "Çok Uzun"]) },
  { id: "char_hair_style", label: "Saç Şekli", type: "select", categoryId: "character", subgroupId: "hair", jsonPath: "hair.style", options: imgOpts("char_hair_style", ["Düz", "Dalgalı", "Kıvırcık", "Afro", "Örgülü", "At Kuyruğu", "Topuz", "Bob", "Pixie", "Dağınık"]) },
  { id: "char_hair_color", label: "Saç Rengi", type: "select", categoryId: "character", subgroupId: "hair", jsonPath: "hair.color", options: colorOpts([["Siyah", "#1C1310"], ["Kahverengi", "#5A3825"], ["Sarı", "#D6B56A"], ["Platin", "#E8E3D3"], ["Beyaz", "#F5F3EE"], ["Gri", "#9B9B93"], ["Gümüş", "#C7C9CC"], ["Kızıl", "#A33A24"], ["Mor", "#6B3FA0"], ["Mavi", "#3B6EA5"], ["Yeşil", "#3F7A4E"], ["Pembe", "#D98CB3"]]) },
  ...
  // ===== 4. Poz & Hareket =====
  { id: "pose_base", label: "Temel Poz", type: "select", categoryId: "pose", subgroupId: "base", jsonPath: "pose.base", options: imgOpts("pose_base", ["Ayakta", "Oturuyor", "Yatıyor", "Diz Çöküyor", "Yürüyor", "Koşuyor", "Zıplıyor", "Dans Ediyor"]) },
  ...
  { id: "cam_angle", label: "Kamera Açısı", type: "select", categoryId: "camera", subgroupId: "angle", jsonPath: "camera.angle", options: imgOpts("cam_angle", ["Göz Hizası", "Alttan Açı", "Üstten Açı", "Kuşbakışı", "Solucan Bakışı", "Eğik Açı", "Omuz Üzerinden"]) },
  ...
  // ===== 23. Fotoğraf =====
  { id: "photo_type", label: "Fotoğraf Türü", type: "select", categoryId: "photo", subgroupId: "type", jsonPath: "photo.type", options: imgOpts("photo_type", ["Portre", "Moda", "Sokak", "Manzara", "Ürün", "Mimari", "Yemek", "Otomotiv", "Düğün", "Belgesel"]) },
